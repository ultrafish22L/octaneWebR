...
| 44  | **Inspector depth shading (WIP/deferred)**              | Medium     | Octane-style progressive background lightening per nesting depth. Depth classes (`depth-0..4`) already in TSX (`ParameterGroup level` prop). CSS needs `background` shorthand (not `background-color`) to override `.node-box` gradient. Debug colors confirmed selectors work. Reverted — resume when ready. |
