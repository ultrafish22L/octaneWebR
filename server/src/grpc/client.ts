...
export class OctaneGrpcClient extends EventEmitter {
  ...
  private loadServiceProto(serviceName: string): any {
    ...
    const serviceToProtoMap: Record<string, string> = {
      ...
      // LocalDB — all three services live in the same proto file
  ...
  public getService(serviceName: string): any {
    ...
    if (!ServiceConstructor && descriptor === this.protoDescriptor) {
      console.log(
        `⚠️ Service ${serviceName} not in main descriptor, trying service-specific proto...`
      );
    ...
    if (typeof ServiceConstructor !== 'function') {
      throw new Error(
        `ServiceConstructor for ${serviceName} is not a constructor (type: ${typeof ServiceConstructor})`
      );
    }
    ...
    if (ServiceConstructor.service) {
      console.log(
        `🔍 Found .service property:`,
        Object.keys(ServiceConstructor.service).slice(0, 10)
      );
    }
  ...
  private resolveServicePath(descriptor: any, path: string): any {
    ...
    console.log(
      `🔍 Searching for path: ${path} in descriptor with keys:`,
      Object.keys(descriptor).slice(0, 20)
    );
    ...
    for (const part of parts) {
      if (current && current[part]) {
        ...
        console.log(
          `  ✅ Found part '${part}', type: ${typeof current}, keys:`,
          Object.keys(current || {}).slice(0, 10)
        );
      } else {
        console.log(
          `  ❌ Part '${part}' not found in current object with keys:`,
          Object.keys(current || {}).slice(0, 20)
        );
        return null;
      }
    }
  ...
  async callMethod(
    ...
    try {
      ...
      if (!method || typeof method !== 'function') {
        ...
        console.log(
          `🔍 All properties on service:`,
          allProps.filter(p => !p.startsWith('_')).slice(0, 20)
        );
    ...
    try {
      ...
      stream.on('data', (response: any) => {
        try {
          ...
          if (
            response.render_images &&
            response.render_images.data &&
            response.render_images.data.length > 0
          ) {
          ...
          // Priority 2: Check if newImage callback has render_images
            ...
            response.newImage?.render_images &&
            response.newImage.render_images.data &&
            response.newImage.render_images.data.length > 0
          ) {
          ...
          // Priority 3: Check alternative field names (renderimages lowercase)
            ...
            response.renderimages &&
            response.renderimages.data &&
            response.renderimages.data.length > 0
          ) {
  ...
  async getMemoryUsage(deviceIx: number = 0): Promise<any> {
    try {
      ...
      console.log(
        `💾 Memory usage for device ${deviceIx}:`,
        JSON.stringify(response.memUsage, null, 2)
      );
