...
| ------------------------------------------ | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Quad primitive (type 18)**               | ECONNRESET on `set_attribute`                   | Octane Alpha 5 bug. Types 1-17, 19-23 are safe. Workaround: flat Box (`A_SCALE` Y near 0.001) or `NT_GEO_MESH` + `quad.obj`.                                    |
| **`evaluate:false` on any node op**        | ECONNRESET after ~8 deferred calls              | NEVER defer evaluations. Always use `evaluate:true` (the default).                                                                                              |
| **`restart_render`**                       | ECONNRESET                                      | Tool was REMOVED from MCP. Use `start_render` once — Octane stays in render mode and picks up changes live.                                                     |
| **Parallel `create_node` calls**           | ECONNRESET after ~4 simultaneous calls          | Create nodes sequentially only. Never parallelize node creation.                                                                                                |
...
| **High subdivision on `NT_GEO_OBJECT`**    | Potential crash or hang                         | Keep subdivision values low.                                                                                                                                    |
...
| `connect_nodes` returns success but nothing changed | Used `pin_name: "Input N"` on geo group  | Use `pin_index: N` (0-based) for geo group dynamic pins. `pin_name` silently fails.                                  |
| Geo group connection does nothing (fresh group)     | Fresh geo groups have 0 input pins       | Set `A_PIN_COUNT` (attr 113) to 4+ BEFORE connecting children.                                                       |
| Medium pin on auto-created environment not working  | Pin not materialized on auto-created env | Create standalone `NT_ENV_TEXTURE`, connect medium FIRST, then connect env to RT.                                    |
...
| `camera_visibility` bool doesn't stick via MCP                         | Reverts to `true` after setting                       | Workaround: disconnect fill geo from group, or position lights behind camera/subjects.                                 |
...
| **Render doesn't update after connect**       | `start_render` does NOT refresh geometry tree                                | Call `update_scene()` then `set_camera` after every structural change. Both are required.                               |
...
| Geo group rejects connections                   | `A_PIN_COUNT` not set on fresh groups                                      | Set `A_PIN_COUNT` (attr 113) BEFORE connecting children. Use `pin_index` (0-based), NOT `pin_name`.                                                                     |
...
| **Quad primitive (type 18) crashes**  | Alpha 5 bug. Use flat Box or `quad.obj` instead.                                                                                                        |
