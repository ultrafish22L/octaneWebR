...
| **Primitive type enum changes (non-deterministic)** | ECONNRESET crash after N `set_attribute` calls changing primitive type enum on `NT_GEO_OBJECT` nodes. Threshold is non-deterministic: crashed at 5, 15, or survived 40+ in different sessions. Only primitive type changes crash — translation/scale/rotation/material changes are safe at 700+ calls. | Octane bug in geometry rebuild path. No reliable workaround — use `import_glb` for non-box geometry when stability matters. Previous "limit to ≤5" rule was based on incomplete data; real threshold varies per session. |
...
| Transform set returns `success:true` but reads back {0,0,0}            | Set `A_TRANSLATION` on the geo object handle instead of its transform child                                                                | `get_node_info(geo)` → pin 3 (transform) → `connected_handle` → set `A_TRANSLATION=172` on **that child**. Octane silently ignores transform attrs on parent nodes.                                                                          |
...
**Architecture note (v2.1.2):** MCP and web UI share the same gRPC interface. Both use Beta 2 method names. `OctaneGrpcClientBase.callMethod()` handles all API version translation (method names + param transforms) — one code path, one compat layer. MCP tools should never use Alpha 5 method names directly.
...
| ~~Stale gRPC channels after Octane restart~~         | ~~Manual Octane kill didn't trigger ECONNRESET (no call in-flight), so MCP kept stale channels. `create_node` returned handle 0 silently.~~ | **Fixed in v2.1.4**: `ensureConnection()` health-checks the gRPC connection after 30s idle. If Octane was killed and restarted, the health check fails and resets all channels/caches automatically.                            |
| ~~Octane hangs on close while dev server connected~~ | ~~Callback stream was an infinite server-streaming RPC. Octane's graceful shutdown waited for it to finish — deadlock.~~                    | **Fixed in v2.1.4**: Callback stream now has a 60s deadline with auto-reconnect. On shutdown, Octane waits at most 60s for the stream to expire, then exits cleanly. gRPC keepalive pings (10s) detect dead connections faster. |
