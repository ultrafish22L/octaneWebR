...
- **Clay mode preserved** — mugshots save the user's clay mode before rendering and restore it afterward. No more silent clay mode reset.
- **Geometric Z→Y override** — if mesh geometry is Z-up (`native_up: "Z"`), the geometric rotation `{90,0,0}` is preserved even if VLM says "upright." VLM only adjusts front-facing (Y rotation) on top of the geometric baseline. This prevents near-spherical objects from losing their axis correction.
...
- **Apply `mesh_info` transforms** — every imported mesh must use the rotation/scale from its `.mesh_info.json` sidecar (generated by `analyze_geo`). Never guess orientation. User-provided rotation is ADDED to sidecar rotation (e.g. sidecar `{90,0,0}` + user `{0,15,0}` = `{90,15,0}`).
...
**Use `setup_lighting` for the full 3-point rig** — it refreshes live transforms from Octane, reads SEGA intent, computes positions from subject bounds (hero+secondary+accent only), creates emissive planes with `visible: false` (camera-invisible by default), and dims the environment automatically. Use `create_light` for individual lights (accents, practicals, glowing objects) — `visible` defaults to `false`. Set `visible: true` only when you want the light geometry itself to appear in the render. Use `set_daylight` to adjust environment without pin-chasing. Use `suggest_lighting` only if you need the recipe without creating nodes.
...
- **Material pin:** `pin_name: "material"` (use pin names, not indices — pin layout varies by node type).
- **Transform pin:** `pin_name: "transform"` (NT_TRANSFORM_VALUE).
- **Auto-wrapping:** Connecting to RT `pin_name: "geometry"` auto-creates placement chain. No manual group needed for single objects.
- **Multi-object:** Create NT_GEO_GROUP, connect each geo to group's dynamic pins, connect group to RT `pin_name: "geometry"`.
- **Primitive type changes work** — all 23 types tested (values 1-23). Use GLB meshes from OTOY Studio for production quality geometry.
...
Two separate clear operations — Octane scene vs AD state. `delete_node` automatically removes entries from the placement DB.
