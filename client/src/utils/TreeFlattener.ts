...
export interface FlattenedNode {
  ...
  isExpanded: boolean;
  uniqueKey: string;
  isFirstChild: boolean; // Whether this node is the first sibling at its level
  isLastChild: boolean; // Whether this node is the last sibling at its level
  ancestorIsLast: boolean[]; // For each ancestor depth, whether that ancestor was the last child
}
...
export function flattenTree(
  ...
  nodes.forEach((node, arrayIndex) => {
    ...
    const isFirstChild = arrayIndex === 0;
    const isLastChild = arrayIndex === nodes.length - 1;
