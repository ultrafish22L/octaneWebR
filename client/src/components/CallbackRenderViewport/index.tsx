...
export const CallbackRenderViewport = React.memo(
  forwardRef<CallbackRenderViewportHandle, CallbackRenderViewportProps>(
    function CallbackRenderViewport(
      ...
      const displayCallbackImage = useCallback((imageData: OctaneImageData) => {
        ...
        try {
          ...
          setFrameCount(prev => {
            ...
            //        Logger.info('🎬 [VIEWPORT] Frame count incremented to:', newCount);
            return newCount;
          });
      ...
      useEffect(() => {
        ...
        const handleNewImage = (data: CallbackData) => {
          ...
          if (data.render_images && data.render_images.data && data.render_images.data.length > 0) {
            Logger.debug('✅ [VIEWPORT] Valid image data received, calling displayCallbackImage');
       ...
       */
      useEffect(() => {
        Logger.info('🎯 [VIEWPORT] Mouse controls useEffect triggered');
        Logger.info('📊 [VIEWPORT] Canvas available:', !!canvasRef.current);
